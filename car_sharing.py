...
class Owner:
    ...
    def add_car_to_rent(self, car_info):
        car = Car(car_info)
        self.contract.add_booking_details(BookingDetails(car))

    def deploy(self, ether, blockchain):
        #deploying the owner on the blockchain
        self.balance -= ether

        #creating a contract to write on a block
        self.contract = SmartContract()
...
class Car_Info:
    def __init__(self, car_id, name, year, owner, reg_no, daily_price):
        self.car_id = car_id
        self.name = name
        self.year = year
        self.owner = owner
